...
export class AstMongo {
  ...
  async connect() {
    if (
      this._connections.config ||
      this._connections.cdr ||
      this._connections.cel
    )
    ...
    if (this._options.urls.config) {
      ...
      this._aor = this.Model<AorDocument, AorModel, AorProperties>(
        connection,
        AorDefinition,
        'Aor',
        options
      );
      this._auth = this.Model<AuthDocument, AuthModel, AuthProperties>(
        connection,
        AuthDefinition,
        'Auth',
        options
      );
      this._endpoint = this.Model<
        EndpointDocument,
        EndpointModel,
        EndpointProperties
      >(connection, EndpointDefinition, 'Endpoint', options);

      options = {
        plugins: this._options.plugins,
        collectionName: 'ps_endpoint_id_ips',
      };
      this._identify = this.Model<
        IdentifyDocument,
        IdentifyModel,
        IdentifyProperties
      >(connection, IdentifyDefinition, 'Identify', options);

      options = {
        plugins: this._options.plugins,
        collectionName: 'ast_config',
      };
      this._static = this.Model<StaticDocument, StaticModel, StaticProperties>(
        connection,
        StaticDefinition,
        'Static',
        options
    ...
    if (this._options.urls.cdr) {
      ...
      const connection = createConnection(this._options.urls.cdr);
      this._cdr = this.Model<CdrDocument, CdrModel, CdrProperties>(
        connection,
        CdrDefinition,
        'Cdr',
        options
    ...
    if (this._options.urls.cel) {
      ...
      const connection = createConnection(this._options.urls.cel);
      this._cel = this.Model<CelDocument, CelModel, CelProperties>(
        connection,
        CelDefinition,
        'Cel',
        options
   ...
   */
  async disconnect() {
    if (this._connections.config) await this._connections.config.close();
    if (this._connections.cdr) await this._connections.cdr.close();
    if (this._connections.cel) await this._connections.cel.close();
  ...
  get Aor(): AorModel {
    if (!this._aor) throw new Error('No Model, not yet initialized');
    return this._aor;
  }
  get Auth(): AuthModel {
    if (!this._auth) throw new Error('No Model, not yet initialized');
    return this._auth;
  }
  get Endpoint(): EndpointModel {
    if (!this._endpoint) throw new Error('No Model, not yet initialized');
    return this._endpoint;
  }
  get Identify(): IdentifyModel {
    if (!this._identify) throw new Error('No Model, not yet initialized');
    return this._identify;
  }
  get Static(): StaticModel {
    if (!this._static) throw new Error('No Model, not yet initialized');
    return this._static;
  }
  get Cdr(): CdrModel {
    if (!this._cdr) throw new Error('No Model, not yet initialized');
    return this._cdr;
  }
  get Cel(): CelModel {
    if (!this._cel) throw new Error('No Model, not yet initialized');
    return this._cel;
  }
