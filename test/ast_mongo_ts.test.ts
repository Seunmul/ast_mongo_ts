...
function deleteId(data: any): object[] {
  if (data.constructor !== Array) return deleteId([data]);

  const results = [];
  for (const obj of JSON.parse(JSON.stringify(data))) {
    const copy: any = {};
    for (const key of Object.keys(obj)) {
      if (key != '_id') copy[key] = obj[key];
    }
    results.push(copy);
  }
  return results;
...
describe('AstMongo.ts with Aor', () => {
  ...
  describe('aor', () => {
    ...
    test('Update it', async () => {
      const result = await ast_mongo.Aor.updateOne(
        { _id: 'alice' },
        { max_contacts: 2 }
      );
      expect(result).toMatchSnapshot();
...
describe('AstMongo.ts with Endpoint Set', () => {
  beforeAll(async () => {
    ...
    await ast_mongo.connect();
    await ast_mongo.Aor.deleteOne({});
    await ast_mongo.Auth.deleteOne({});
    await ast_mongo.Endpoint.deleteOne({});
  ...
  test('create an endpoint set #1', async () => {
    ...
    const result = await ast_mongo.Endpoint.findOne({ _id: '6001' }).populate(
      'auth'
    );
    expect(result).toMatchSnapshot();
  });
...
describe('AstMongo.ts with StaticConfig', () => {
  ...
  test('add dial plans as StaticConfigs', async () => {
    const properties = await smh.create('extensions.conf', 'context1', [
      { exten: '_6.,1,NoOp()' },
  ...
  test('add identify as StaticConfigs', async () => {
    const properties = await smh.create('pjsip.conf', 'identify', [
  ...
  test('read back extensions', async () => {
    const results = await ast_mongo.Static.find({
  ...
  test('delete extensions', async () => {
    const result = await ast_mongo.Static.deleteOne({
      filename: 'extensions.conf',
    });
    expect(result).toMatchSnapshot();
  });
  test('read back to check', async () => {
    const results = await ast_mongo.Static.find().sort('cat_metric var_metric');
    expect(deleteId(results)).toMatchSnapshot();
  });
  test('delete pjsip', async () => {
    const result = await ast_mongo.Static.deleteOne({ filename: 'pjsip.conf' });
    expect(result).toMatchSnapshot();
  });
  test('read back to check #2', async () => {
    const results = await ast_mongo.Static.find().sort(
      'cat_metric var_metric _id'
    );
    expect(deleteId(results)).toMatchSnapshot();
  });
...
describe('AstMongo.ts with Cdr & Cel', () => {
  ...
  describe('cdr', () => {
    beforeAll(async () => {
      const data = [
        {
          ...
          dst: '6001',
          dcontext: 'context1',
          channel: 'PJSIP/6002-00000000',
          dstchannel: 'PJSIP/6001-00000001',
          lastapp: 'Dial',
          lastdata: 'PJSIP/6001',
          disposition: 'NO ANSWER',
          amaflags: 'DOCUMENTATION',
          accountcode: '',
          uniqueid: 'asterisk-0.0',
          userfield: '',
          peeraccount: '',
          linkedid: 'asterisk-0.0',
          duration: 0,
          billsec: 0,
          sequence: 0,
        ...
        },
        {
          ...
          dst: '6003',
          dcontext: 'context1',
          channel: 'PJSIP/6004-00000000',
          dstchannel: 'PJSIP/6003-00000001',
          lastapp: 'Dial',
          lastdata: 'PJSIP/6003',
          disposition: 'NO ANSWER',
          amaflags: 'DOCUMENTATION',
      ...
      await ast_mongo.Cdr.deleteOne({});
      const results = await ast_mongo.Cdr.create(data);
    });

    test('Read them back', async () => {
      const results = await ast_mongo.Cdr.find().sort('start').select('-_id');
      ...
      const data = [
        {
          ...
          exten: '6001',
          context: 'context1',
          channame: 'PJSIP/6002-00000004',
          appname: '',
          appdata: '',
          accountcode: '',
          peeraccount: '',
          uniqueid: 'asterisk-0.4',
          linkedid: 'asterisk-0.4',
          userfield: '',
          peer: '',
          extra: '',
          eventtime: new Date('2017-01-01T09:12:17.629Z'),
        },
        {
          eventtype: 5,
          eventname: 'APP_START',
          ...
          peer: '',
          extra: '',
          eventtime: new Date('2017-01-01T09:12:17.630Z'),
        },
      ];
      await ast_mongo.connect();
      // await ast_mongo.Cel.deleteOne({});
      const results = await ast_mongo.Cel.create(data);
    });
    test('Read log', async () => {
      const results = await ast_mongo.Cel.find()
        .sort('eventtime')
        .select('-_id');
      expect(results).toMatchSnapshot();
    });
