 ...
 */
import { AstMongo, AorModel, AuthModel, EndpointModel } from '../ast_mongo';
import {
  AorProperties,
  AuthProperties,
  EndpointProperties,
...
export class EndpointSetHelper {
  ...
  constructor(astMongo: AstMongo, cbs?: EndpointSetHelperCB) {
    if (!astMongo.Aor || !astMongo.Auth || !astMongo.Endpoint)
      throw new Error(
        'Any (or all) of Aor, Auth and Endpoint are not available.'
      );
  ...
  async create(id: string, eps: EndpointSet): Promise<void> {
    if (eps.aor && !(eps.aor as any)._id) (eps.aor as any)._id = id;
    if (eps.auth && !(eps.auth as any)._id) (eps.auth as any)._id = id;
    if (eps.endpoint) {
      if (!(eps.endpoint as any)._id) (eps.endpoint as any)._id = id;
      if (!eps.endpoint.aors) eps.endpoint.aors = id;
      if (!eps.endpoint.auth) eps.endpoint.auth = id;
    }

    if (this._cbs && this._cbs.createWith) eps = this._cbs.createWith(eps);

    if (eps.aor) await this._aor.create(eps.aor);
    if (eps.auth) await this._auth.create(eps.auth);
    if (eps.endpoint) await this._endpoint.create(eps.endpoint);
  }
}
